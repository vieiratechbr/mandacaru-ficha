...
export default async function DashboardAdmin() {
  // Proteção: só admin com cookie válido
  ...
  const s: Record<string, React.CSSProperties> = {
    ...
    grid:  { display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:'1rem', maxWidth:860, marginBottom:'2rem' },
    ...
    badge: (s: boolean): React.CSSProperties => ({
      fontSize:'0.6rem', padding:'2px 8px', letterSpacing:'0.15em',
      background: s ? 'rgba(61,92,46,0.3)' : 'rgba(196,154,60,0.2)',
      color: s ? '#3d5c2e' : '#c49a3c', textTransform:'uppercase' as const,
    }),
    viewBtn: { color:'#c49a3c', fontSize:'0.72rem', textDecoration:'none', letterSpacing:'0.1em' },
  }

  const total = fichas?.length ?? 0
  const seladas = fichas?.filter(f => f.selada).length ?? 0
  const emCriacao = total - seladas
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.grid}>
        ...
        <div style={s.stat}><div style={s.num}>{emCriacao}</div><div style={s.slabel}>Em Criação</div></div>
      </div>
          ...
          <tbody>
            {fichas.map((f: any) => (
              <tr key={f.id}>
                ...
                <td style={s.td}><span style={s.badge(f.selada)}>{f.selada ? '🔒 Selada' : '✎ Criando'}</span></td>
